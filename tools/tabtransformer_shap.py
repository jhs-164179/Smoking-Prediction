...
def forfinal(df):
    ...
    model = TabTransformer(
        categories=tuple(uniques),
        num_continuous=len(numeric_col),
        dim=30,dim_out=1,depth=4,heads=8,attn_dropout=0.44974240860832937,ff_dropout=0.44974240860832937,mlp_hidden_mults=(4,2),
        mlp_act=nn.SELU(inplace=True),
        continuous_mean_std=torch.tensor(combined_array, dtype=torch.float32)
    )
    model = model.to(device)
    optim = Adam(model.parameters(), lr=.0001)
    ...
    for epoch in range(epochs):
            ...
            if val_f1 > best_f1:
                best_f1 = val_f1
                best_epoch = epoch+1
                torch.save(model.state_dict(), './bestTab.pth')
                
    print(f'Best Epoch: {best_epoch} | Best F1 : {best_f1:.4f}')  
    return model, test_set, cat_col, numeric_col